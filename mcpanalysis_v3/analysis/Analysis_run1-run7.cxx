...
//	cout<<"-------------------------Get Vars for Analysis------------------------------------------"<<endl;
	...
	if(t[5]>1600 || t[6]>1600)		return;
	if(q[1]<1 || q[1]>3)				return;
	if(q[2]<1 || q[2]>3)				return;
...
/*	//--------------------------- reference time resolution ---------------------------
	...
	f->cd("raw");
	int ir = 0;
	double xmin[4] = {0, 0, 700, 700};
	double xmax[4] = {4, 4, 1600, 1600};
	while(ir<5) {
		if(ir == 0){
			cout<<"=======================... raw time resolution ... =======================" << endl;
			for(int ichan=0;ichan<4;ichan++){
				Calibration *docalib = new Calibration(CalibX[ichan], CalibY[1], ir, ichan, ProfileMode, BinSizeMode, 50, xmin[ichan], xmax[ichan], 100,-500, 500); 
