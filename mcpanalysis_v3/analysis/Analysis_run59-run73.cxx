...
//	if(q[3]>q[1] && q[3]>q[2])			return;
	if(t[5]<500 || t[5]>1200 || t[6]<500 || t[6]>1200)		return;
	if(q[1]<10 || q[1]>600)	return;
	if(q[2]<10 || q[2]>600)	return;
//	if(q[1]<40 || q[1]>90)				return; //3200V
//	if(q[2]<40 || q[2]>90)				return; //3200V
...
//	tabs->Fit("f1", "qr", "", para[1]-3*para[2], para[1]+3*para[2]);
	...
	f->cd("raw");
	int ir = 0;
	double xmin[4] = {400, 400, 800, 800};
	double xmax[4] = {600, 600, 900, 900};
	while(ir<5) {
		if(ir == 0){
